/* ============================================================
   ...
   Fluxo: intro → dados pessoais → 8 perguntas → resultado + feedback → envio
...
function renderHeader(passoAtual) {
  ...
  header.innerHTML = `
    <div>
      <div class="eyebrow">Inteli · Quiz v2</div>
...
function baixarBackupLocal(payload) {
  ...
  a.href = url;
  a.download = `quiz-v2-${(flow.nome || "resposta").replace(/\s+/g, "_")}.json`;
  a.click();
